...
const firstLetters = (arr) => {
  // Solution code here...
  let mapLetters = arr.map(letters => {
...
const findHappiness = (arr) => {
  // Solution code here...
  let filter = arr.filter( letters => {
    if(letters.includes(':)')) return true;
  })
  return filter;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let format = arr.map(nums => {
    let area = nums.substring(1, 4);
    let firstThree = nums.substring(6, 9);
    let secondThree = nums.substring(10, 14);
    return area + firstThree + secondThree;
  })
  return format;
};
