...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let vow = arr.filter(vowels =>{
    let find = /\w*[aeiou]\w*|[aeiou]/g
    let findVow = vowels.match(find);
    return findVow;
  });
  return vow;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let allArr = arr.filter (arg => {
    if(forbiddenValues.includes(arg) === false) return arg;
  });
  return allArr;
};
