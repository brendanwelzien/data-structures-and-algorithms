...
const templateWithJQuery = () => {
  $(starWarsPeople).each(person =>{
    let $copyCharacter = $('#template').clone();
    $copyCharacter.find('h2').text(starWarsPeople[person].name);
    $copyCharacter.find('h3').text(starWarsPeople[person].height);
    $copyCharacter.find('p').text(starWarsPeople[2].eye_color); // wants red eye color
    $('main').append($copyCharacter);
  });
};
...
const listFoods = (recipe) => {
  ...
  recipeItems.forEach( v => {
    let stringSlice = v.slice(' ');
    let index = stringSlice.indexOf(' ');
