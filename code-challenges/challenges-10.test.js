...
const count = (target, input) => {
  // Solution code here...
  let int = 0;
  let mapOne = input.map (valueOne => {
    let mapTwo = valueOne.map (valueTwo => {
      if( valueTwo === target ){
        int++;
      }
    });
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filter = input.filter(valueOne => {
    let map = valueOne.map (valueTwo => {
      if(valueTwo % 5 === 0){
        return valueTwo;
      }
      return Math.pow(2, valueOne);
    });
  });
};
